...
These Spectre Scan Terms & Conditions of Supply together with the documents referred to in it ("Terms") constitute the terms and conditions on which Sarosys OOD ("Licensor") will grant to any purchaser or user ("Licensee") a licence to use the software comprising Spectre Scan ("Spectre Scan" or the "Software"), following acceptance of an order as detailed below.

The following expressly form part of the Terms:

- The Spectre Scan License Agreement;
...
1. Licenses to Spectre Scan are available for purchase via the Licensor's website at https://ecsypno.com/products/scnr.

2. Placing an order for Spectre Scan or checking "I have read and accept the terms and conditions" on a webform is an offer by the Licensee to purchase a licence to the Software and does not constitute a contract until such time as the Licensor issues an email or web confirmation that the order is accepted by Sarosys OOD.  Notwithstanding the foregoing, by installing the Software the Licensee affirms that it agrees to the terms of the License and the Spectre Scan terms and conditions of supply, which bind the Licensee and its employees.  The contract will only relate to the Software the Licensee has licensed, as set out in that confirmation or accepted by installing it.  Notwithstanding any other communications between the parties, ordering and/or downloading the Software by the Licensee, or the download of the Software by another party at the instigation of the Licensee, shall constitute conclusive evidence that the Licensee has purchased the Software on the basis of these Terms & Conditions of Supply and Sarosys OOD's order quotation.
...
4. If the Licensee's license is subject to auto-renewal and the Licensee has provided a valid recurring payment method, then payment will be taken 14 days before the expiry of the then current License Period (as defined in the Spectre Scan License Agreement) ("Renewal Payment Date"). Auto-renewal for specific licences can be disabled on the Licensee's account page.
...
7. When the contract for the purchase of Spectre Scan has been concluded, such contract is made for the benefit of the Licensee and Sarosys OOD only and is not intended to benefit, or be enforceable by, anyone else.
...
Spectre Scan License Agreement
...
1.3. Each installation of Spectre Scan on an individual computer needs to be activated before it will operate, including subsequent Licenses commenced using auto-renewal. It is recognised that in the course of ordinary usage of the Software, individual users may need to install the product on more than one computer. The number of activations performed for each licence is monitored. The Licensor reserves the right to limit the number of activations allowed per licence, and to prevent further activations if this limit is exceeded.
...
echo
echo
echo -n "Spectre Scan installed at:   "
echo $scnr_dir
echo "Installation log at: $log"
echo
echo "* For a CLI scan you can run: $scnr_dir/bin/spectre URL"
